...
const FACTORS = [
  ...
  { key: 'engagement',      label: 'Disengagement from wellness check-ins', max: 6 },
  ...
  { key: 'disciplinary',    label: 'Recent disciplinary markers',       max: 8 }
];
// theoretical max 120 -> normalized to 100
...
function computeRisk(d, config = {}) {
  ...
  const onDepNow = deployments.length && days(today, dparse(deployments[deployments.length - 1].date)) <= 180;
  if (onDepNow || deployments.length >= 2) {
    out.push({ key: 'deployment', points: deployments.length >= 2 ? 12 : 7,
      detail: deployments.length >= 2 ? `${deployments.length} deployments in past year` : 'On extended deployment (>90 days)' });
  }
  ...
  const disc = d.hr.filter(e => e.type === 'disciplinary' && days(today, dparse(e.date)) <= 120);
  if (disc.length) {
    out.push({ key: 'disciplinary', points: Math.min(8, disc.length * 4),
      detail: `${disc.length} disciplinary event${disc.length > 1 ? 's' : ''} in 120 days` });
  }
...
function addWellnessFactors(d, today, out) {
  ...
  const c30 = c.filter(x => Math.round((today - new Date(x.date + 'T00:00:00Z').getTime()) / 86400000) <= 30).length;
  if (c30 <= 3 && d.personnel.active) {
    out.push({ key: 'engagement', points: 6,
      detail: `Only ${c30} wellness check-ins in past 30 days` });
  }
  ...
  if (worst >= 60) {
    out.push({ key: 'assessment', points: Math.min(14, 7 + (worst - 60) / 4),
      detail: `Wellness assessment flagged at ${worst}/100` });
  }
}
